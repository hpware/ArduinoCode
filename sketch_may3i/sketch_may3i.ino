...
const int I2S_DMA_BUF_LEN = 1024;
  ...
  const int StreamBufferLen = StreamBufferNumBytes / 4; // Number of 32-bit raw samples
  int32_t StreamBuffer[StreamBufferLen]; // Buffer for raw 32-bit samples
#else // Assuming 16-bit raw samples
...
const uint16_t WAV_BITS_PER_SAMPLE = 16; // Output WAV will be 16-bit
typedef struct {
    char RIFF[4] = {'R', 'I', 'F', 'F'};
    uint32_t ChunkSize;
    char WAVE[4] = {'W', 'A', 'V', 'E'};
...
File audioFile; 
const char* audioFileName = "/mic_recording_plus20.wav"; 
...
void setup() {
    ...
    Serial.print("Opening file on SD card for writing: "); Serial.println(audioFileName);
...
void updateWavHeader(File file, uint32_t totalDataSize) {
    ...
    if (!file.seek(4)) { Serial.println("FAILED."); file.seek(currentPos); return; }
    Serial.print("OK. Writing ChunkSize... ");
    size_t written = file.write((const uint8_t*)&chunkSize, sizeof(chunkSize));
    if (written != sizeof(chunkSize)) { Serial.println("FAILED."); file.seek(currentPos); return; }
    Serial.println("OK.");

    Serial.print("Seeking to offset 40 (Subchunk2Size)... ");
    if (!file.seek(40)) { Serial.println("FAILED."); file.seek(currentPos); return; }
     Serial.print("OK. Writing Subchunk2Size... ");
    written = file.write((const uint8_t*)&subchunk2Size, sizeof(subchunk2Size));
     if (written != sizeof(subchunk2Size)) { Serial.println("FAILED."); file.seek(currentPos); return; }
    Serial.println("OK.");

    Serial.print("Seeking back to position "); Serial.print(currentPos); Serial.print("... ");
    if(!file.seek(currentPos)) { Serial.println("FAILED."); } else { Serial.println("OK."); }
